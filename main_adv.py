...
import losses_lt_new
import preact_resnet_models as resnet_models
...
flags.DEFINE_integer('eval_freq', 250, 'eval frequency')
...
flags.DEFINE_string('loss', 'adv_shift',
...
flags.DEFINE_float('n_it_update', 1000, 'frequency of g, lambda updates')
...
def main(_):
  ...
  all_len = np.sum(samples_per_cls)
  if FLAGS.metric_base == 'uniform':
    ...
    g_y = samples_per_cls
  g_y = g_y / np.sum(g_y) 
  p_y = [val / all_len for val in samples_per_cls]
  pi_y = g_y
  alpha_y = [1 for i in samples_per_cls] # TOOD: alpha_y denotes for 
  ...
  loss_op = losses_lt_new.MakeLossFunc(FLAGS.loss, samples_per_cls, pi_y, p_y, FLAGS.gamma,
...
#               lambda <- lambda - eta_lambda * gradient(Lagrangian)_lambda
  ...
  def update_adv(ds, p_y, pi_y, r_list, lambd, delta,
              eta_g=0.01, eta_lambda=0.01, weight_type=FLAGS.weight_type):
    ...
    num_y_list = [0] * FLAGS.num_classes
    iterator = iter(ds)
    kl_val = sum(rel_entr(pi_y, p_y))
    if 0.1 > kl_val: # TODO: set r=0.1
      alpha = 0
    else:
      alpha = 0.5
    for batch in iterator:
      ...
      #       model prediction is: argmax_y (alpha_y * softmax_y)
      y_pred_prob = tf.nn.softmax(y_pred, axis=-1)
      y_weighted_pred = y_pred_prob 
      y_weighted_pred = y_weighted_pred / tf.reduce_sum(y_weighted_pred,
                                                        1, keepdims=True)
      arg_pred = tf.argmax(y_weighted_pred, axis=1,
                           output_type=tf.int32)
    ...
    # Continue with accumulated loss for the whole validation set
    loss_y_list /= tf.cast(num_y_list, dtype=tf.float32)
    loss_y = tf.clip_by_value(loss_y_list, clip_value_max=2)
    # TODO
    g_pi = []
    tmp = loss_y.numpy().tolist()
    for kk in range(len(p_y)):
      g_pi.append(tmp[i] / p_y[i])
    pi_y_updated = []
    eps = 0.001
    for kk in range(len(p_y)):
      part1 = (pi_y[i] * (p_y[i] ** alpha)) ** (1 / (1+alpha))
      part2 = math.exp(0.1 * g_pi[i])
      pi_y_updated.append(part1 * part2 + eps)
    return pi_y_updated / np.sum(pi_y)
  ...
  for it in range(1, FLAGS.num_iters + 1):
    ...
    images, labels = batch['image'], batch['label']
    # pi_y = update_adv(ds=valid_ds, p_y=p_y, pi_y=pi_y, lambd=lambd, delta=FLAGS.eps, r_list=r_list, eta_g=FLAGS.eta_g, eta_lambda=FLAGS.eta_lambda)
    loss, _ = train_step(images, labels, it)
    # if FLAGS.loss == 'adv_shift':
    if it % FLAGS.n_it_update == 0 and FLAGS.loss == 'adv_shift':
      pi_y = update_adv(ds=valid_ds, p_y=p_y, pi_y=pi_y, lambd=lambd, delta=FLAGS.eps, r_list=r_list, eta_g=FLAGS.eta_g, eta_lambda=FLAGS.eta_lambda)
      loss_op = losses_lt_new.MakeLossFunc(FLAGS.loss, samples_per_cls, pi_y, p_y, FLAGS.gamma,
                                       FLAGS.beta, FLAGS.s, FLAGS.tau)
